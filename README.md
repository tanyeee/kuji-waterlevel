...
地点一覧と取得IDは `config/stations.json` にまとめています。`stations` はデータ更新・保存を続ける全地点、`display_groups` はWebアプリに表示する非潮汐地点です。
...
- `display_groups`: Webアプリの観測地点セレクトに表示する河川名と地点ID。ここにない地点も取得・保存は続けます
...
榊橋上・高橋・下石崎は、水文水質データベースではなく「川の防災情報」が画面表示に使用する公開JSONから取得します。

```bash
python scripts/update_recent_from_kawabou_files.py --keep-10min-hours 168 --keep-hourly-days 45
```

観測所コードは `ofcCd` 5桁、`itmkndCd` 3桁、`obsCd` 5桁をゼロ埋めして連結します。現在値JSONの `min10Values` と `hrValues`、日別JSONの `pastValues` を既存形式へ変換します。公開側では長期履歴が保持されないため、取得後はこのリポジトリの既存JSONへ追記し、1時間値は従来のマージ処理で `historical_hourly.json` に移します。

| 地点 | 水系・河川 | `ofcCd` | `obsCd` | 観測所コード |
| --- | --- | ---: | ---: | --- |
| 榊橋上 | 久慈川・久慈川 | 21271 | 3 | `2127100400003` |
| 高橋 | 那珂川・涸沼川 | 2049 | 1 | `0204900400001` |
| 下石崎 | 那珂川・涸沼川 | 21271 | 23 | `2127100400023` |

この3地点は追加以前の長期履歴がないため、直近3年の基準線と水位判定を「データ蓄積中」として無効化します。実測グラフと選択期間の最低・平均・最高は表示します。
...
画面では1つの観測地点セレクトに河川名のグループ見出しを付けます。潮汐の影響を受ける地点は選択肢から除外しますが、潮汐アプリなどが利用できるようデータ更新と保存は全地点で継続します。

支流の機初（`303011283322080`、里川）と常井橋（`303011283322070`、山田川）も2016年以降の1時間履歴と直近10分値を使用し、既存の更新workflowで取得します。地点間では水位の基準面が異なるため、水位の絶対値ではなく各地点の増減や増水時刻を比較してください。水位だけで支流の流量や魚の活性を直接判定するものではありません。
