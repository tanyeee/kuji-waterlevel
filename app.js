...
const LEVEL_ZONE_STYLE = {
  borderColor: '#f4c96b',
  fillColor: 'rgba(244,201,107,.12)',
  hatchColor: 'rgba(255,224,154,.22)',
  hatchSpacing: 10
};

const levelZonePlugin = {
  id: 'levelZones',
  beforeDatasetsDraw(chartInstance) {
    const zones = getLevelZones();
    ...
    for (const zone of zones) {
      const thresholdY = Math.max(
        chartArea.top,
        Math.min(chartArea.bottom, yScale.getPixelForValue(zone.from))
      );
      const height = thresholdY - chartArea.top;
      if (height <= 0) continue;

      ctx.save();
      ctx.beginPath();
      ctx.rect(chartArea.left, chartArea.top, chartArea.right - chartArea.left, height);
      ctx.clip();
      ctx.fillStyle = LEVEL_ZONE_STYLE.fillColor;
      ctx.fillRect(chartArea.left, chartArea.top, chartArea.right - chartArea.left, height);
      ctx.strokeStyle = LEVEL_ZONE_STYLE.hatchColor;
      ctx.lineWidth = 1;
      ctx.beginPath();
      for (
        let x = chartArea.left - height;
        x < chartArea.right;
        x += LEVEL_ZONE_STYLE.hatchSpacing
      ) {
...
const els = {
  ...
  levelZoneNote: document.getElementById('levelZoneNote'),
...
const LIVE_WATERLEVEL_BASE = 'https://tanyeee.github.io/kuji-waterlevel-data/live/stations';
const HOURLY_ARCHIVE_BASE = 'https://raw.githubusercontent.com/tanyeee/kuji-waterlevel-data/main/data';
...
async function fetchRecentTenMinute(station) {
  try {
    ...
    console.warn('Live 10-minute data unavailable.', error);
    return { meta: { source: 'optional-empty', record_count: 0 }, records: [] };
  }
}

function expectedHourlyCount(year) {
  return (year % 4 === 0 && (year % 100 !== 0 || year % 400 === 0) ? 366 : 365) * 24;
}

function recordsFromHourlyYear(payload, stationId, year) {
  const expectedCount = expectedHourlyCount(year);
  if (payload?.schemaVersion !== 1 || payload.station !== stationId || payload.year !== year ||
      payload.stepMinutes !== 60 || !Array.isArray(payload.values) || payload.values.length !== expectedCount) {
    throw new Error(`Invalid hourly archive: ${stationId}/${year}`);
  }

  const flags = payload.flags || {};
  return payload.values.map((value, index) => {
    const hour = new Date(Date.UTC(year, 0, 1, index));
    const timestamp = `${hour.getUTCFullYear()}-${String(hour.getUTCMonth() + 1).padStart(2, '0')}-${String(hour.getUTCDate()).padStart(2, '0')}T${String(hour.getUTCHours()).padStart(2, '0')}:00`;
    return {
      timestamp,
...
async function fetchHourlyArchiveManifest() {
  if (!hourlyArchiveManifestPromise) {
    hourlyArchiveManifestPromise = fetchJson(`${HOURLY_ARCHIVE_BASE}/manifest.json`).catch(error => {
      hourlyArchiveManifestPromise = null;
      throw error;
    });
  }
  return hourlyArchiveManifestPromise;
}

async function fetchArchivedHourly(station) {
  if (!hourlyArchiveCache.has(station.id)) {
    const request = (async () => {
      const manifest = await fetchHourlyArchiveManifest();
      const years = manifest.hourlyYears?.[station.id];
      if (!Array.isArray(years) || years.length === 0) {
        throw new Error(`No archived hourly years for ${station.id}`);
      }

      const yearlyPayloads = await Promise.all(years.map(year =>
        fetchJson(`${HOURLY_ARCHIVE_BASE}/hourly/${station.id}/${year}.json`)
      ));
      const records = yearlyPayloads.flatMap((payload, index) =>
        recordsFromHourlyYear(payload, station.id, years[index])
      );
      return {
        meta: {
          source: 'annual_hourly_archive',
          record_count: records.length,
          dataset_start: records[0]?.timestamp || null,
          dataset_end: records[records.length - 1]?.timestamp || null
        },
        records
      };
    })();
    hourlyArchiveCache.set(station.id, request);
  }

  try {
    return await hourlyArchiveCache.get(station.id);
  } catch (error) {
...
async function fetchHistoricalHourly(station) {
  return fetchArchivedHourly(station);
}
...
function updateStationCopy() {
  ...
  const levelZones = getLevelZones();
  els.levelZoneNote.hidden = levelZones.length === 0;
  els.levelZoneNote.textContent = levelZones.map(zone => zone.note).filter(Boolean).join(' ');
}

function getLevelZones(station = currentStation) {
  return (station?.level_zones || []).filter(zone => Number.isFinite(zone?.from));
}
...
function generateLineLegendLabels(chart) {
  ...
  const labels = Chart.defaults.plugins.legend.labels.generateLabels(chart).map(item => {
    ...
    if (dataset.levelZone) {
      return {
        ...label,
        fillStyle: LEVEL_ZONE_STYLE.fillColor,
        strokeStyle: LEVEL_ZONE_STYLE.borderColor,
        lineDash: [],
        lineWidth: 1,
        pointStyle: 'rect',
        levelZone: true
...
function render() {
  ...
  const visibleFloodLevels = els.toggleFloodLines.checked
    ...
    : [];
  const levelZoneThresholds = levelZones.map(zone => zone.from);
  const levelZoneCeilings = levelZones.map(zone => zone.from + 0.15);
  ...
  for (const zone of levelZones) {
    datasets.push({
      label: zone.label,
      data: buildLineSeries(records, zone.from),
      borderColor: LEVEL_ZONE_STYLE.borderColor,
      borderDash: [7, 5],
      borderWidth: 1.6,
      pointRadius: 0,
      levelZone: true
